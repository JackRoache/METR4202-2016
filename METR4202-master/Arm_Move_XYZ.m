...
for i=1:size(points,1)-1;
    ...
    for j=0:pointNum
        ...
        if((max(abs(solution-lastSolution)) > 50) && ~(j==0 && i==1))
...
%             y = currPoint(2) + (j-1)/pointNum *(nextPoint(2)-currPoint(2));
            ...
            angleSet2 = Arm_Find_Solution(x,y);
            %find the solution closest to the phi = 90*direction
            [Y,I] = min(abs(angleSet2(:,1) - lastSolution(1)));
            solution2 = angleSet2(I,:);
            
            Arm_Set_Speeds(100,100,100,100,PORTNUM);
            Arm_Set_Angles(solution2(1),solution2(2),solution2(3),endAngle,PORTNUM);
            ...
            %Moved from top of  function
            Arm_Set_Angles(300,300,300,0,PORTNUM);
            if(Arm_Wait_Moving(4,3,PORTNUM))
                warning('timeout @ switching direction 1')
                Arm_Set_Angles(300,300,300,0,PORTNUM);
                Arm_Wait_Moving(4,1.5,PORTNUM);
            end
...
%             pause(0.5);
            ...
            if(Arm_Wait_Close(solution,solution2,0.9,PORTNUM))
                warning('timeout @ switching direction 2')
                Arm_Set_Speeds(100,100,100,100,PORTNUM);
                Arm_Set_Angles(solution(1),solution(2),solution(3),0,PORTNUM);
            end
