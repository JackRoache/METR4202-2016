function exp_array=expand_array(node_x,node_y,hn,xTarget,yTarget,CLOSED,MAX_X,MAX_Y, obstacles)
    ...
    %obstacles = {[2,2];[1,2];[4,5];[4,2];[4,3];[4,4];[3,5];[2,5];[5,1];[5,2];[2,3]};
    exp_array=[];
    exp_count=1;
            ...
            if (k~=j || k~=0)  %The node itself is not its successor
                ...
                if( (s_x >0 && s_x <=MAX_X) && (s_y >0 && s_y <=MAX_Y))%node within array bound
                    for l=1:size(obstacles,1)                        
                        if((node_x == obstacles{l}(1)) && (s_y == obstacles{l}(2)))
                            flag=0;
                            break;
                        else
                            flag=1;
                        end
                        if((s_x == obstacles{l}(1)) && (node_y == obstacles{l}(2)))
                            flag=0;
                            break;
                        else
                            flag=1;
